...
import { validateKey } from "./utilities/validators.js";
...
export default class Ephemeris {
  ...
  CalculateKeys() {
    return !!this._key && this._key.length // if key array
      ? Body.KeysExceptEarth.filter((b) => this._key.includes(b.key)).map((b) =>
          this.CalculateBody(b.key)
        )
  ...
  CalculateBody(bodyKey) {
    ...
    switch (body.type) {
      ...
      case "heliocentric":
        return new HeliocentricOrbitalBody(
          body,
          this.Earth,
          this.Observer,
      ...
      default:
        throw new Error(
          `Celestial body with key: "${bodyKey}" or type "${body.type}" not found.`
        );
        break;
    }
  }
  ...
  static CalculateDailyBody({
    ...
    const startDate = new Date(
      Date.UTC(startYear, startMonth, startDay, hours, minutes, 0)
    );
    const endDate = new Date(
      Date.UTC(endYear, endMonth, endDay, hours, minutes, 0)
    );
    const datesArray = [];
