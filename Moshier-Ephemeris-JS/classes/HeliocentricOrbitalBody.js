import { B1950, RTD } from "../constants/index.js";

import { aberration } from "../utilities/aberration.js";
import { altaz } from "../utilities/altaz.js";
import { constellation } from "../utilities/constellation.js";
import { deflection } from "../utilities/deflection.js";
import Epsilon from "../utilities/Epsilon.js";
import { kepler } from "../utilities/kepler.js";
import { light } from "../utilities/light.js";
import { lonlat } from "../utilities/lonlat.js";
import { nutation } from "../utilities/nutation.js";
import { precess } from "../utilities/precess.js";
import { util } from "../utilities/util.js";
import {
  getApparentLongitude,
  getDirectedDate,
  getApparentLongitudeDifference,
  calculateNextRetrogradeStation,
  calculateNextDirectStation,
} from "../utilities/motion.js";
...
export default class HeliocentricOrbitalBody {
  constructor(
    ...
    this._body = this.calculateBody(
      ...
      this._earthBody,
      this._observer,
      calcHeliocentricLongitudesOnly
    );
  ...
  calculateBody(
    ...
    return this.reduceBody(
      body,
      body.position.rect,
      earthBody.position.rect,
      earthBody,
      observer,
      calcHeliocentricLongitudesOnly
    );
  ...
  calculateMotion(body, observer, calculateShadows = false) {
    ...
    this.motion.oneSecondMotionAmount = getApparentLongitudeDifference(
    ...
    if (calculateShadows) {
      if (!this.motion.isRetrograde) {
        const nextRetrograde = calculateNextRetrogradeStation({
          direction: "next",
          utcDate: observer.Date.utc,
          bodyKey: body.key,
        ...
        this.motion.nextRetrogradeApparentLongitude =
          nextRetrograde.apparentLongitude;

        const nextDirect = calculateNextDirectStation({
          direction: "next",
          utcDate: nextRetrograde.date,
          bodyKey: body.key,
          currentApparentLongitude: nextRetrograde.apparentLongitude,
          currentMovementAmount: nextRetrograde.nextMovementAmount,
        });
      ...
      this.motion.withinPreRetrogradeShadow = !this.motion.isRetrograde
        ? this.position.apparentLongitude >=
      ...
      if (!this.motion.isRetrograde && !this.motion.withinPreRetrogradeShadow) {
        ...
        // to save resources
        const prevRetrograde = calculateNextRetrogradeStation({
          direction: "prev",
          utcDate: observer.Date.utc,
          bodyKey: body.key,
          currentApparentLongitude: this.position.apparentLongitude,
        ...
        this.motion.prevRetrogradeApparentLongitude =
          prevRetrograde.apparentLongitude;

        const prevDirect = calculateNextDirectStation({
          direction: "next",
          utcDate: prevRetrograde.date,
          bodyKey: body.key,
          currentApparentLongitude: prevRetrograde.apparentLongitude,
          currentMovementAmount: prevRetrograde.nextMovementAmount,
        });
      ...
      this.motion.withinPostRetrogradeShadow =
        !this.motion.isRetrograde && !this.motion.withinPreRetrogradeShadow
          ? this.position.apparentLongitude >=
  ...
  reduceBody(
    ...
    if (!calcHeliocentricLongitudesOnly) {
      ...
      body.position.equatorialDiameter =
        (2.0 * body.semiDiameter) / body.locals.EO;
      ...
      s =
        body.magnitude +
        2.1715 * Math.log(body.locals.EO * body.locals.SO) -
        1.085 * Math.log(b);
