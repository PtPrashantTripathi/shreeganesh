...
const KalachakraGroup KalachakraDasaExpert::K_NAKSHATRA_GROUP[27] =
	{
		KG_ASWINI, KG_BHARANI, KG_ASWINI, KG_ROHINI, KG_Mrigashira, KG_Mrigashira, KG_ASWINI, KG_BHARANI, KG_ASWINI,
		KG_ROHINI, KG_Mrigashira, KG_Mrigashira, KG_ASWINI, KG_BHARANI, KG_ASWINI, KG_ROHINI, KG_Mrigashira, KG_Mrigashira,
		KG_ASWINI, KG_BHARANI, KG_ASWINI, KG_ROHINI, KG_Mrigashira, KG_Mrigashira, KG_ASWINI, KG_BHARANI, KG_ASWINI};
...
bool KalachakraDasaExpert::isMarkatiField(const int &group, const int &pada, const int &column)
{
	...
	{
		if (
			MARKATI_LOCATIONS[i][group] == group && MARKATI_LOCATIONS[i][pada] == pada && MARKATI_LOCATIONS[i][column] == column)
			return true;
	}
	return false;
}
...
KalachakraGati KalachakraDasaExpert::calculateGati(const Rasi &ad_lord, const Rasi &oldlord, const bool &markatiField)
{
	...
	if (oldlord != R_NONE)
	{
		...
		{
		case R_ARIES:
			if (oldlord == R_SAGITTARIUS)
				gati = KdgSimhavalokan;
			break;
		case R_GEMINI:
			if (oldlord == R_LEO)
				gati = KdgMandooka;
		...
		if (column > 8)
		{
			...
			if (config->vedicCalculation->kalachakraMode == 0)
			{
				...
				else
				{
					...
					if ((savya && K_NAKSHATRA_GROUP[nakshatra] > 1) || (!savya && K_NAKSHATRA_GROUP[nakshatra] <= 1))
						nakshatra -= 3;
					if (nakshatra < 0)
						nakshatra += 27;
					group = K_NAKSHATRA_GROUP[nakshatra];
...
wxString KalachakraDasaExpert::getDasaLordName(const int &lord, const TEXT_FORMAT &format, WriterConfig *c)
{
	...
	if (cfg->signSymbols)
	{
		...
		return lang.getSignName((Rasi)lord, format);
}
...
MString KalachakraDasaExpert::getDasaDescriptionF(Dasa *dasa, const TEXT_FORMAT format, WriterConfig *cfg)
{
	...
	if (kdasa->getDasaLord() == jeeva)
		suffix = _("Jeeva");
	if (kdasa->getDasaLord() == deha)
		suffix = _("Deha");
	if (kdasa->getDasaLord() == amsa)
		suffix = _("Amsa");
			...
			for (column = 0; column < 9; column++)
			{
				...
				for (j = 0; j < 9; j++)
				{
					...
					if (i == 0) // cut off at the beginning (antardasas before starting point)
					{
						...
						if (dist < -300)
							dist += 360; // correct if 360-deg shift
						if (dist < 0)
							append = false;
					}
					if (i >= 28) // cut off at the end (if event belongs to next time interval)
					{
						...
						if (dist < -300)
							dist += 360;
						if (dist > 0)
							append = false; // could also be return ?
					}
					if (append == true)
					{
