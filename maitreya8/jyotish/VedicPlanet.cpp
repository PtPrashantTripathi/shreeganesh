	  ...
	  index(pindex)
{
	...
	for (int division = (int)V_RASI; division < NB_VARGAS; division++)
		vargadata[division] = new VargaData(this, (Varga)division);
}
...
void VedicPlanet::calculateDignities()
{
	...
	for (Varga varga = V_FIRST; varga <= V_LAST; varga++)
	{
		...
		/*
		 * NB: Moon in Taurus is exalted AND in Mulatrikona (Varga Chart, e.g. D-9). So exaltation will prevail because of order below.
		 * Same for Saturn in Aquarius (own and Mulatrikona): Mulatrikona will prevail.
		 */
		// Own sign
		...
		// Special handling for Mulatrikona: Rasi depends on exact degree, other Vargas only on sign position
		if (varga == V_RASI)
		{
			if (::isInMulatrikona(index, horoscope->getVedicLongitude(index)))
			{
				dignity = DG_MulaTRIKONA;
				bala = 45;
			}
		}
		else
...
void VedicPlanet::calculateVimsopakaBala()
{
	...
	for (i = 0; i < 6; i++)
		addVimsopakaValue(shadvarga_weight[i], shadvarga_vargas[i], 0);
	for (i = 0; i < 7; i++)
		addVimsopakaValue(saptavarga_weight[i], saptavarga_vargas[i], 1);
	for (i = 0; i < 10; i++)
		addVimsopakaValue(dasavarga_weight[i], dasavarga_vargas[i], 2);
	for (i = 0; i < 16; i++)
		addVimsopakaValue(shodasavarga_weight[i], shodasavarga_vargas[i], 3);
...
void VedicPlanet::addVimsopakaValue(const double &weight, const Varga &varga, const int &vindex)
{
	...
	if (getVargaData(varga)->getVargaViswa() >= minbala)
		vimsopakaBalaGoodVargas[vindex]++;
}
