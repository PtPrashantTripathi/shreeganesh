...
Horoscope::Horoscope() : BasicHoroscope()
{
	...
	for (i = 0; i < NUM_LAGNA; i++)
		lagna_len[i] = 0;
	for (i = 0; i < NUM_UPA; i++)
		upagraha_len[i] = 0;
	for (p = FIRST_EPHEM_OBJECT; p <= LAST_EPHEM_OBJECT; p++)
		ephem_planets[p] = new VedicPlanet(this, p);
	...
	for (i = 0; i <= 8; i++)
	{
		...
		for (j = 0; j < 6; j++)
		{
			if (config->vedicCalculation->kalavela.lord[j] == lord)
			{
				...
				if (config->vedicCalculation->kalavela.portion[j] == KALAVELA_PORTION_MIDDLE)
					thejd += .5 * step_len;
				else if (config->vedicCalculation->kalavela.portion[j] == KALAVELA_PORTION_END)
					thejd += step_len;
...
void Horoscope::updateObjectPositions()
{
	...
	for (p = OSUN; p <= OSATURN; p++)
		ephem_planets[p]->calculateDignities();
...
{
	ASSERT_VALID_OBJECT(planet)
	if (IS_EPHEM_OBJECT(planet) || IS_HOUSE_OBJECT(planet))
		return BasicHoroscope::getObjectPosition(planet, vedic);
	else if (planet <= LAST_EXTENDED_OBJECT)
		return (vedic ? *evpos[planet - FIRST_EXTENDED_OBJECT] : *ewpos[planet - FIRST_EXTENDED_OBJECT]);
	else
		return arabic_expert->getObjectPosition(planet, vedic);
}
