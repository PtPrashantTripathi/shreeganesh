...
wxString Lang::getNakshatraName(const int &nak, const bool &use28, const TEXT_FORMAT format) const
{
	const static wxString k_nakshatra_name_large[28] = {_("Aswini"), _("Bharani"), _("Krittika"), _("Rohini"),
														_("Mrigashira"), _("Ardra"), _("Punarvasu"), _("Pushyami"), _("Aslesha"), _("Magha"),
														_("Poorvaphalguni"), _("Uttaraphalguni"), _("Hasta"), _("Chitra"), _("Swati"), _("Visakha"),
														_("Anuradha"), _("Jyeshta"), _("Mula"), _("Poorvashadha"), _("Uttarashadha"), _("Abhijit"),
														_("Sravana"), _("Dhanishta"), _("Satabhisha"), _("Poorvabhadra"), _("Uttarabhadra"), _("Revati")};
	const static wxString k_nakshatra_name_medium[28] = {
		_("Aswini"), _("Bharani"), _("Krittika"), _("Rohini"), _("Mrigashira"), _("Ardra"),
		...
		_("Mula"), _("P.Shadha"), _("U.Shadha"), _("Abhijit"), _("Sravana"), _("Dhanishta"),
	...
	if (format == TF_SHORT)
		return k_nakshatra_name_short[index];
	if (format == TF_MEDIUM)
		return k_nakshatra_name_medium[index];
	else
		return k_nakshatra_name_large[index];
}
...
const wxString Lang::getObjectName(const ObjectId &num, const TEXT_FORMAT &format, const bool vedic) const
{
	...
	if (num < MAX_EPHEM_OBJECTS)
	{
		...
		{
			if (num == OTRUENODE || num == OMEANNODE)
				ret = _("Rahu");
			if (num == OTRUEDESCNODE || num == OMEANDESCNODE)
				ret = _("Ketu");
		}
		if (writercfg->vedicPlanetNames)
		{
			if (num <= OSATURN)
				ret = k_object_large_vedic[num];
			else if (num == OASCENDANT)
				ret = k_object_large_vedic[7];
		}
	}
	...
	if (format == TF_MEDIUM)
	{
		if (!vedic)
		{
			if (num == OTRUENODE || num == OMEANNODE)
				ret = wxT("Nod");
			else if (num == OTRUEDESCNODE || num == OMEANDESCNODE)
				ret = wxT("DNo");
		}
		return ret.Left(3);
	}
...
const wxString Lang::getFriendshipName(const FRIENDSHIP &f, const TEXT_FORMAT format) const
{
	...
	if (format == TF_SHORT)
		return k_friendship_short[f];
	else
		return writercfg->vedicPlanetNames ? k_friendship_sk[(int)f] : k_friendship[(int)f];
}
...
const wxString Lang::getDignityName(const DIGNITY &d, const TEXT_FORMAT format) const
{
	...
	const static wxString k_dignity_name_sk[4] = {_("Uchcha"), _("Nicha"), _("Mulatrikona"), _("Svakshetra")};
	const static wxString k_dignity_name[4] = {_("Exalted"), _("Debilated"), _("Mulatrikona"), _("Own")};
	...
	if (format == TF_SHORT)
		return k_dignity_short[index];
	else
		return writercfg->vedicPlanetNames ? k_dignity_name_sk[(int)index] : k_dignity_name[(int)index];
}
...
const wxString Lang::getSignName(const Rasi &i, const TEXT_FORMAT format) const
{
	...
	if (format == TF_LONG)
	{
		if (writercfg->vedicSignNames)
			s = k_sign_full_vedic[i];
		else
			s = k_sign_full[i];
	}
...
const wxString Lang::getObjectSignification(const ObjectId &p, const TEXT_FORMAT &, const bool vedic)
{
	...
	if (vedic && p >= OSUN && p <= OSATURN)
		return k_vedic_signification[int(p)];
	else
		return wxEmptyString;
};
...
{
	if (a)
		return wxString::Format(wxT("%d"), a);
	else
		return wxEmptyString;
}
